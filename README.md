...
이 레포의 `install_all.sh`, `codex/install.sh`, `claude-code/install.sh`, `cleanup_backups.sh`, `verify_install.sh`에 있는 `set -euo pipefail`, whitelist, `exit 1`은 유지합니다. 이 스크립트들은 연구 실험 코드가 아니라 홈 설정과 백업 삭제를 다루므로, 과한 프로덕션 방어가 아니라 사용자 데이터 보호 장치입니다.
