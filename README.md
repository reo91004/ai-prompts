...
macOS 기본 Bash 3.2, Linux, WSL을 지원합니다. Native Windows와 Git Bash는 지원 범위가 아닙니다. Codex 또는 Claude Code CLI가 설치된 환경에서는 Node.js, `git`, 네트워크 연결을 사용해 LazyCodex와 Ponytail도 함께 설치하며 LazyCodex 설치에는 `npx`가 추가로 필요합니다. LazyCodex는 Codex에만 설치하고, Ponytail은 발견된 두 호스트에 각각 설치합니다. 외부 통합을 의도적으로 생략해야 하는 오프라인·테스트 환경에서는 `UNIVERSAL_RESEARCH_AGENT_KIT_SKIP_INTEGRATIONS=1`을 지정할 수 있습니다.
...
설치 스크립트는 기존 설정과 agents/skills 디렉터리를 전용 상태 디렉터리에 백업한 뒤, ownership manifest에 기록된 키트 소유 항목만 교체합니다. LazyCodex, Ponytail, 개인 스킬처럼 다른 이름을 사용하는 제3자 항목은 삭제하지 않습니다. 키트 소유 항목과 이름이 같은 파일이나 디렉터리는 백업 후 키트 버전으로 교체합니다.
...
정책은 `Global Core → Domain Skill → Project Overlay` 순서로 적용됩니다. 작은 로컬 작업은 main agent가 단독 처리합니다. 위임을 선택한 경우 서로 다른 산출물을 맡는 child agent를 최소 2개 사용하며, detector 결과가 동시성 1이면 두 child를 순차 실행합니다. child의 nested delegation은 금지됩니다.

resource detector는 macOS의 `sysctl`, `memory_pressure`, `vm_stat`, Linux/WSL의 `/proc`, PSI, cgroup v2 신호를 읽습니다. Linux에서는 현재 process의 cgroup v2 경로와 mount root를 해석하고 그 경로부터 mount root까지 가장 엄격한 memory, CPU, cpuset, OOM 제한을 적용합니다. 사용 가능 메모리 구간과 CPU 한도, task cap, `HARNESS_MAX_THREADS` 중 최솟값을 사용하며 기본 상한은 6입니다. `HARNESS_TASK_CAP`으로 wave별 상한을 더 낮출 수 있습니다. low swap, swapout/OOM 증가, critical PSI, 감지 실패 또는 600초를 넘긴 snapshot은 동시성을 1로 제한하거나 구조화된 RESOURCE 오류로 중단합니다. writer slot과 heavy-command slot은 각각 하나이고 `max_depth = 1`입니다. Linux snapshot은 `--system-root`로 read-only replay할 수 있습니다.
...
이 키트는 Sequential Thinking MCP가 이미 설치되어 있다고 가정합니다. genuinely hard planning, 원인이 불명확한 debugging, 비싼 실험 설계, claim acceptance에서만 사용하며 MCP 부재를 성공으로 가장하지 않습니다. LazyCodex와 Ponytail은 통합 설치 대상이지만 Sequential Thinking MCP 같은 기타 MCP 서버는 별도로 관리합니다. 기존 user-global `arxiv`, `semantic-scholar`, `semantic_scholar` MCP 등록은 제거하고 Zotero는 보존합니다.
