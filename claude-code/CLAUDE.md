...
- Record requested role, declared model and effort, spawn transport, and any `CLAUDE_CODE_SUBAGENT_MODEL` or `CLAUDE_CODE_EFFORT_LEVEL` override in every task packet. Record effective role, model, and effort only when runtime evidence exposes them; otherwise use the literal value `unverified` rather than copying declarations into evidence.
